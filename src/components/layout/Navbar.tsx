...
export const Navbar = () => {
  ...
  useEffect(() => {
    fetch("https://api.desigrocery.ie/api/store/locations/")
      .then(res => res.json())
      .then(data => {
        setLocations(data);
        const urlLocId = searchParams.get('location');
    ...
    fetch("https://api.desigrocery.ie/api/announcements/")
      .then(res => res.json())
      .then(data => setAnnouncements(data))
      .catch(err => console.error("Failed to load announcements", err));
  }, []);
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      fetch(`https://api.desigrocery.ie/api/store/products/?search=${searchQuery}&location=${selectedLocation.id}`)
        .then(res => res.json())
        .then(data => setSearchResults(data.slice(0, 5)))
        .catch(err => console.error("Search failed", err));
    }, 300);
