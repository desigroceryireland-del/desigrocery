...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const productUrl = `https://api.desigrocery.ie/api/store/products/${locationId ? `?location=${locationId}` : ''}`;

    Promise.all([
      fetch("https://api.desigrocery.ie/api/store/categories/").then(res => res.json()),
  ...
  }, [locationId]); // ✅ 3. Re-run fetch whenever the location changes in the URL
        ...
        <div className="container py-12 lg:py-20">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                <Link to={`/category/all${locationId ? `?location=${locationId}` : ''}`}>
                  <Button size="lg" className="bg-primary hover:bg-saffron-dark text-primary-foreground gap-2">
                    Shop Now
                    <ArrowRight className="h-5 w-5" />
                  </Button>
                </Link>
                {/* Order for Pickup maintains location */}
                <Link to={`/category/all${locationId ? `?location=${locationId}` : ''}`}>
                  <Button size="lg" variant="outline" className="border-secondary text-secondary hover:bg-secondary hover:text-secondary-foreground">
                    Order for Pickup
                  </Button>
      ...
      <section className="py-16">
        <div className="container">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to={`/offers${locationId ? `?location=${locationId}` : ''}`}>
              <Button variant="outline" className="hidden sm:flex gap-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground">
                View All Offers <ArrowRight className="h-4 w-4" />
              </Button>
            </Link>
