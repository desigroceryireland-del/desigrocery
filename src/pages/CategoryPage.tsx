...
const CategoryPage = () => {
  ...
  // Fetch Categories
  useEffect(() => {
    fetch("https://api.desigrocery.ie/api/store/categories/")
      .then(res => res.json())
      .then(data => setCategories(data))
      .catch(err => console.error("Error fetching categories:", err));
  }, []);

  // ✅ 2. Re-fetch products when locationId, categoryId, or subcategories change
  useEffect(() => {
    setLoading(true);

    let url = "https://api.desigrocery.ie/api/store/products/?";
    if (activeCategory) url += `category=${activeCategory}&`;
    if (selectedSubcategories.length === 1) url += `subcategory=${selectedSubcategories[0]}&`;
    
    // ✅ 3. Attach locationId to the request to get correct pricing
  ...
  }, [activeCategory, selectedSubcategories, locationId]); // ✅ Added locationId as dependency

  const currentCategory = categories.find(c => c.slug === activeCategory);

  const filteredProducts = useMemo(() => {
    let filtered = [...products];
    // Filters based on the price returned by the backend for this location
    filtered = filtered.filter(p => p.price >= priceRange[0] && p.price <= priceRange[1]);
    if (offersOnly) filtered = filtered.filter(p => p.in_offer);
    if (minRating > 0) filtered = filtered.filter(p => p.rating >= minRating);
  ...
  // ✅ 4. Maintain the location query param when changing categories
  const handleCategoryChange = (slug: string | undefined) => {
    const locQuery = locationId ? `?location=${locationId}` : "";
    if (!slug || slug === 'all') {
      navigate(`/category/all${locQuery}`);
    } else {
      navigate(`/category/${slug}${locQuery}`);
    }
  };
  ...
  // ... (Keep the rest of your JSX logic for rendering Categories and Products)
if (categoryId === 'all') {
    return (
      <Layout>
        <div className="container py-8">
